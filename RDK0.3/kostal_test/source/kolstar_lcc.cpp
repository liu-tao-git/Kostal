...
std::stack<std::array<double, 13>> data_stack;
std::stack<std::array<uint8_t, 16>> SPI_stack;
std::stack<std::array<double, 13>> data_stack_rev;
...
Struct Quaternion_to_Euler (double x, double y, double z, double w)
{
    double Roll = atan((1 - 2 * (x * x + y * y))/(2 * (w * x + y * z))); 
    double Pitch = asin(2 * (w * y - x * z));
    double Yaw = atan((1 - 2 * (z * z + y * y))/2 * (w * z + x * y));
    
    
    Struct Eout;
    {
    Eout.Roll = Roll/3.1415*360;
    Eout.Pitch = Pitch/3.1415*360;
    Eout.Yaw = Yaw/3.1415*360;
    }
    return Eout;
}
    ...
    std::shared_ptr<flexiv::Robot> robot)
{
    ...
    data_stack.push({outdata[0],outdata[1],outdata[2],outdata[3],outdata[4],outdata[5],outdata[6],outdata[7],outdata[8],outdata[9],outdata[10],outdata[11],outdata[12]});
    {
...
int main(int argc, char* argv[])
{
    ...
    //std::thread robthread(std::bind(hptask,robotStates,robot));
    
    
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));


    robot->executePlanByName(argv[1]);               
    ...
    {
    data_stack_rev.push({data_stack.top()[0],data_stack.top()[1],data_stack.top()[2],data_stack.top()[3],data_stack.top()[4],data_stack.top()[5],data_stack.top()[6],data_stack.top()[7],data_stack.top()[8],data_stack.top()[9],data_stack.top()[10],data_stack.top()[11],data_stack.top()[12]});
    ...
    std::string filename="./data/"+std::string(argv[1])+time_str+".csv";
    MyExcelFile.open(filename);
    
    //add file headers  
    MyExcelFile << "x"<<","<<"y"<<"," << "z"<<"," ;
    MyExcelFile << "Rx"<<","<<"Ry"<<"," << "Rz"<<"," ;
    ...
    MyExcelFile << std::endl; 
    //MyExcelFile << data_stack.top()[0] << ", "<< data_stack.top()[1]<< ", "<< data_stack.top()[2]<< ", "<< data_stack.top()[3]<< ", "<< data_stack.top()[4]<< ", "<< data_stack.top()[5] << ", " << data_stack.top()[6]<< ", "<< data_stack.top()[7]<< ", " << data_stack.top()[8] <<", "<< data_stack.top()[9]<<","<< SPI_stack.top()[0]<< ", "<< SPI_stack.top()[1] << ", "<< SPI_stack.top()[2] << ", "<< SPI_stack.top()[3]<< ", "<< SPI_stack.top()[4]<< ", "<< SPI_stack.top()[5]<< ", "<< SPI_stack.top()[6]<< ", "<< SPI_stack.top()[7]<< ", "<< SPI_stack.top()[8]<< ", "<< SPI_stack.top()[9]<< ", "<< SPI_stack.top()[10]<< ", "<< SPI_stack.top()[11]<< ", "<< SPI_stack.top()[12] << ", "<< SPI_stack.top()[13]<< ", "<< SPI_stack.top()[14]<< ", "<< SPI_stack.top()[15]  << std::endl;
    //", "<< data_stack.top()[2] <<  ", "<< data_stack.top()[3] << ", "<< data_stack.top()[4] <<  ", "<< data_stack.top()[5] <<  ", "<< data_stack.top()[6] <<  ", "<< data_stack.top()[7] <<  ", "<< data_stack.top()[8] <<  ", "<< data_stack.top()[9] <<  ", "<< SPI_stack.top()[0] ", "<< SPI_stack.top()[1] ", "<< SPI_stack.top()[2] ", "<< SPI_stack.top()[3] ", "<< SPI_stack.top()[4] ", "<< SPI_stack.top()[5] ", "<< SPI_stack.top()[6] ", "<< SPI_stack.top()[7] ", "<< SPI_stack.top()[8] ", "<< SPI_stack.top()[9] ", "<< SPI_stack.top()[10] ", "<< SPI_stack.top()[11] ", "<< SPI_stack.top()[12] ", "<< SPI_stack.top()[13] ", "<< SPI_stack.top()[14] ", "<< SPI_stack.top()[15] << std::endl;  
    
    //toBinary(showspi); //testfunction

    //check syncsats
    printf("%02X ",showspi);
    std::cout<< ' ' <<showdata[0]<< std::endl;
    data_stack_rev.pop();
    SPI_stack_rev.pop();
