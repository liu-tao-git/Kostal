/**
 * @example kstar.cpp
 * Select a plan from a list to execute using RDK's plan execution API.
 * @copyright (C) 2016-2021 Flexiv Ltd. All Rights Reserved.
 * @author Flexiv
 */

#include <Robot.hpp>
#include <Log.hpp>
#include <string>
#include <iostream>
#include <cmath>
#include <mutex>
#include <thread>
#include "ControlSPI.h"
#include <stdio.h>
#include <iostream>
#include <unistd.h>
#include <stack>
#include <array>
#include <fstream>



namespace {

unsigned int g_printCounter = 0;


//temp_robot_data 
double outdata[11] ={0};
//temp_SPI_data
uint8_t SPIout[16] = {0};
//uint8_t SPIout_tmp[16] = {0};
//uint8_t showspi[16]={0};
int ret;


//stack for data storage
std::stack<std::array<double, 10>> data_stack;
std::stack<std::array<uint8_t, 16>> SPI_stack;




struct PrintData
{
    std::vector<double> tcp_pose;
    std::vector<double> raw_sensor_data;

} g_printData;

std::mutex g_printDataMutex;
std::mutex SPImutex;
}

//test function uint8 to binary 
void toBinary(uint8_t a)
{
    uint8_t i;

    for(i=0x80;i!=0;i>>=1)
        printf("%c",(a&i)?'1':'0'); 
}



void highPriorityPeriodicTask(std::shared_ptr<flexiv::RobotStates> robotStates,
    std::shared_ptr<flexiv::Robot> robot)
{

    robot->getRobotStates(robotStates.get());

    // save data to global variable for printing in another thread
    {
        std::lock_guard<std::mutex> lock(g_printDataMutex);
        g_printData.tcp_pose = robotStates->m_tcpPose;
        g_printData.raw_sensor_data = robotStates->m_rawDataFromForceSensor;        
    }
    outdata[0]=g_printData.tcp_pose[3];
    outdata[1]=g_printData.tcp_pose[4];
    outdata[2]=g_printData.tcp_pose[5];
    outdata[3]=g_printData.tcp_pose[6];

    outdata[4]=g_printData.raw_sensor_data[0];
    outdata[5]=g_printData.raw_sensor_data[1];
    outdata[6]=g_printData.raw_sensor_data[2];
    outdata[7]=g_printData.raw_sensor_data[3];
    outdata[8]=g_printData.raw_sensor_data[4];
    outdata[9]=g_printData.raw_sensor_data[5];
        //printf("go\n");

    
    data_stack.push({outdata[0],outdata[1],outdata[2],outdata[3],outdata[4],outdata[5],outdata[6],outdata[7],outdata[8],outdata[9]});
    {
    std::lock_guard<std::mutex> lock(SPImutex);
    SPI_stack.push({SPIout[0],SPIout[1],SPIout[2],SPIout[3],SPIout[4],SPIout[5],SPIout[6],SPIout[7],SPIout[8],SPIout[9],SPIout[10],SPIout[11],SPIout[12],SPIout[13],SPIout[14],SPIout[15]});
    }

    
}

void hptask(std::shared_ptr<flexiv::RobotStates> robotStates,
    std::shared_ptr<flexiv::Robot> robot)
{
robot->start(std::bind(highPriorityPeriodicTask, robotStates, robot));
    //printf("go\n");

}




void SPIdata_collection()  
{
  while (true){
        uint8_t read_buffer[10240] = {0};
        int32_t read_data_num = 0;
        ret = VSI_SlaveReadBytes(VSI_USBSPI, 0, read_buffer, &read_data_num,100);
        if (ret != ERR_SUCCESS){
            printf("Read data error!\n");
            
        }else{
            if (read_data_num == 16){
                //printf("Read data num: %d\n",read_data_num);
                //printf("\n");
                {
                std::lock_guard<std::mutex> lock(SPImutex);
                for (int i = 0; i < read_data_num; i++){
                    //printf("%02X ",read_buffer[i]);
                    SPIout[i]=read_buffer[i];
                }
                }
            }else{
                ///printf("No data!\n");
            }
             
    }   
}
}

int main(int argc, char* argv[])
{

    flexiv::Log log;
        // IP of the robot server
    std::string robotIP = "192.168.2.100";

    // IP of the workstation PC running this program
    std::string localIP = "192.168.2.104";

    // RDK Initialization
    //=============================================================================
    // instantiate robot interface
    auto robot = std::make_shared<flexiv::Robot>();

    // create data struct for storing robot states
    auto robotStates = std::make_shared<flexiv::RobotStates>();

    // initialize robot interface and connect to the robot server
    robot->init(robotIP, localIP);

    // wait for the connection to be established
    do {
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    } while (!robot->isConnected());

    // enable the robot, make sure the E-stop is released before enabling
    if (robot->enable()) {

        log.info("Enabling robot ...");
    }

    // wait for the robot to become operational
    do {
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    } while (!robot->isOperational());
    log.info("Robot is now operational");


    VSI_INIT_CONFIG SPI_Config;
    // Scan connected device
    ret = VSI_ScanDevice(1);
    if (ret <= 0) {
        printf("No device connect!\n");
        return ret;
    }
    // Open device
    ret = VSI_OpenDevice(VSI_USBSPI, 0, 0);
    if (ret != ERR_SUCCESS) {
        printf("Open device error!\n");
        return ret;
    }

    // Initialize device(Slave Mode, Hardware SPI, Full-Duplex)
    SPI_Config.ControlMode = 0;
    SPI_Config.MasterMode = 0; // Slave Mode
    SPI_Config.CPHA = 1; // Clock Polarity and Phase must be same as master
    SPI_Config.CPOL = 0;
    SPI_Config.LSBFirst = 0;
    SPI_Config.TranBits = 8; // Support 8bit mode only
    SPI_Config.SelPolarity = 0;
    SPI_Config.ClockSpeed = 1395000;
    ret = VSI_InitSPI(VSI_USBSPI, 0, &SPI_Config);
    if (ret != ERR_SUCCESS) {
        printf("Initialize device error!\n");
        return ret;
    }
    printf("SPI Initialize device successfully!\n");
    // initialize robot interface and connect to the robot server
    
    robot->setMode(flexiv::MODE_PLAN_EXECUTION);
    
    do {
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
    } while (robot->getMode() != flexiv::MODE_PLAN_EXECUTION);
         
    

    std::thread SPIthread(SPIdata_collection);
        //printf("go\n");

    std::thread robthread(std::bind(hptask,robotStates,robot));
    
    robot->executePlanByName(argv[1]);               


    flexiv::SystemStatus systemStatus;
    robot->getSystemStatus(&systemStatus);

    do {
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
    robot->getSystemStatus(&systemStatus);

    } while (systemStatus.m_programRunning == true);

    //printf("go\n");
    
    std::ofstream MyExcelFile;
    std::string filename="./data/"+std::string(argv[1])+".csv";
    MyExcelFile.open(filename);
    


    std::cout<< ' ' <<data_stack.size() << std::endl;
    std::cout<< ' ' <<SPI_stack.size() << std::endl;

    while (!data_stack.empty())            
    {
    //printf("go\n");

    auto showdata = data_stack.top();           
    uint8_t showspi = SPI_stack.top()[6];           
   
    MyExcelFile << data_stack.top()[0] << ", "<< data_stack.top()[1]<< ", "<< data_stack.top()[2]<< ", "<< data_stack.top()[3]<< ", "<< data_stack.top()[4]<< ", "<< data_stack.top()[5] << ", " << data_stack.top()[6]<< ", "<< data_stack.top()[7]<< ", " << data_stack.top()[8] <<", "<< data_stack.top()[9]<<","<< SPI_stack.top()[0]<< ", "<< SPI_stack.top()[1] << ", "<< SPI_stack.top()[2]<< ", "<< SPI_stack.top()[3]<< ", "<< SPI_stack.top()[4]<< ", "<< SPI_stack.top()[5]<< ", "<< SPI_stack.top()[6]<< ", "<< SPI_stack.top()[7]<< ", "<< SPI_stack.top()[8]<< ", "<< SPI_stack.top()[9]<< ", "<< SPI_stack.top()[10]<< ", "<< SPI_stack.top()[11]<< ", "<< SPI_stack.top()[12] << ", "<< SPI_stack.top()[13]<< ", "<< SPI_stack.top()[14]<< ", "<< SPI_stack.top()[15]  << std::endl;
    //", "<< data_stack.top()[2] <<  ", "<< data_stack.top()[3] << ", "<< data_stack.top()[4] <<  ", "<< data_stack.top()[5] <<  ", "<< data_stack.top()[6] <<  ", "<< data_stack.top()[7] <<  ", "<< data_stack.top()[8] <<  ", "<< data_stack.top()[9] <<  ", "<< SPI_stack.top()[0] ", "<< SPI_stack.top()[1] ", "<< SPI_stack.top()[2] ", "<< SPI_stack.top()[3] ", "<< SPI_stack.top()[4] ", "<< SPI_stack.top()[5] ", "<< SPI_stack.top()[6] ", "<< SPI_stack.top()[7] ", "<< SPI_stack.top()[8] ", "<< SPI_stack.top()[9] ", "<< SPI_stack.top()[10] ", "<< SPI_stack.top()[11] ", "<< SPI_stack.top()[12] ", "<< SPI_stack.top()[13] ", "<< SPI_stack.top()[14] ", "<< SPI_stack.top()[15] << std::endl;  
    
    //toBinary(showspi); 
    //printf("%02X ",showspi);
    //std::cout<< ' ' <<showdata[0]<< std::endl;
    data_stack.pop();
    SPI_stack.pop();


    }

    MyExcelFile.close();
    
    return 0;
}
